# [Ola 1.01] SWAL-01 — feat(theme-engine): Dynamic JSON theme engine and CLI dispatcher
...
- Hardcoded SCSS styling in eww.scss without dynamic theme switching.
- No CLI or schema validation for desktop color palettes.

## Desired State (DELTA)
- Implement `scripts/swal-theme` with list, switch, create, and current commands.
- Implement `schemas/theme.schema.json` adhering to JSON Schema Draft-07.

## 🌐 Web Research Required
**MANDATORY — 4-6 queries. The agent MUST research before implementing.**
1. search: "python json schema validation best practices"
2. search: "hyprctl keyword color reload dynamically"
3. search: "eww reload without restart"
4. search: "linux cli theme switcher pattern"
...
- Python 3 stdlib CLI with argparse
- Atomic JSON file writing and graceful error handling

## Acceptance Criteria (VERIFIABLE BY COMMAND)
- [ ] `test -x scripts/swal-theme`
- [ ] `python3 scripts/swal-theme list | grep -q "hive-dark"`
- [ ] `test -f schemas/theme.schema.json`
- [ ] `git status --porcelain` shows expected files in the assigned island

## Files to Modify
| File | Current State | Change | Risk |
|---|---|---|---|
| `scripts/swal-theme` | 0 lines | New dynamic theme manager executable | LOW |
| `schemas/theme.schema.json` | 0 lines | New JSON Schema for themes | LOW |
